...
function hackathonCard(item) {
    ...
    return `<div class="col-lg-4 col-md-6"><article class="card h-100 border-0 shadow-sm"><div class="card-body d-flex flex-column">
      ...
      <details class="small mb-3"><summary class="text-primary fw-semibold">Why am I seeing this?</summary><ul class="text-muted ps-3 mt-2">${(recommendation.reasons || []).slice(0, 3).map(item => `<li><strong>${escapeHtml(item.source.replaceAll('_',' '))}:</strong> ${escapeHtml(item.evidence)}</li>`).join('') || '<li>Complete your profile for more specific reasons.</li>'}</ul></details>
